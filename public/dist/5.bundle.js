...
RegisterHtmlTemplate.register("<dom-module id=authors-list> <template> <style include=shared-styles>:host{display:block;padding:10px}iron-list{height:100vh;max-width:500px}</style> <luding-search data={{data}} responseprop=authors queryfunction=[[getBody]]> </luding-search> <iron-list id=list items=[[data]] as=item> <template> <luding-card item=[[item]] godetails=[[navigate]] objid=authorid> <span slot=title>[[inc(index)]]. [[item.authorGivName]] [[item.authorLastName]]</span> <span slot=subtitle>[[item.count]] Spiele </span> <paper-icon-button icon=icons:arrow-forward on-tap=toggleSelection> </paper-icon-button> </luding-card> </template> </iron-list> </template> </dom-module>");
