...
RegisterHtmlTemplate.register("<dom-module id=publisher-details> <template> <style include=shared-styles>:host{display:block;padding:10px}iron-list{height:100vh;max-width:500px}</style> <app-location route={{route}} path={{path}}></app-location> <iron-ajax url=/api/graphql on-response=handleResponse content-type=application/json handle-as=json method=POST body=[[body]] auto=\"\"></iron-ajax> <h1>[[data.publishername]]</h1> <div> <games-listfragment data=[[data.publishergames]]></games-listfragment> </div> </template> </dom-module>");
